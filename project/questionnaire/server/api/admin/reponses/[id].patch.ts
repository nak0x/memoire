...
// Seul le dépouillement est modifiable : codage de la Q9 et notes de lecture.
// Les réponses elles-mêmes ne sont jamais réécrites.
export default defineEventHandler(async (event) => {
  ...
  const res = useDb()
    .prepare('UPDATE responses SET codes = ?, notes = ? WHERE id = ?')
